...
Framing classification (meshtastic/lorawan/unknown) exists now: `model::classifyLoraFrame` (2026-09-14) is a structural best-effort guess — Meshtastic's fixed-size header + broadcast marker, LoRaWAN's MHDR + length invariants, both cited against the real source/spec, not guessed. It is **not hardware-confirmed against real traffic**: the only live traffic seen so far is MeshCore, which isn't in DESIGN's classified set at all and is expected to (correctly) read as `unknown` given this classifier's signal — that expectation itself hasn't been checked against a real captured MeshCore payload, since field data is never committed (SECURITY.md) and none was available off-bench. Worth a deliberate look next bench session: confirm real MeshCore traffic doesn't false-positive as `meshtastic` (the design keeps this to a documented ~2^-32 risk via the broadcast-address check, but that's arithmetic, not a bench observation yet).
