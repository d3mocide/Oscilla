...
- [x] Model: current-fix service feeding the logger. `GnssModel` exists with the exact fields DESIGN §9.1's struct calls for (`lat/lon/alt/hdop/utc/valid` + derived `age_ms`), plus a full calendar date (`year/month/day`, parsed from RMC — GGA carries none) added 2026-09-14 for the wardrive CSV's FirstSeen column. Nothing reads it yet — no view, no logger wired to it. That wiring is unstarted, not just unverified.
...
- [x] Wardrive: stream observations, deck-side geotag against local fix + age, write WigleWifi CSV + KML; **Drive** view. **Format writers only, started early/out of sequence like the Contacts/Deauth/Spectrum batch above** (2026-09-14): `storage::wardriveCsv*` (WigleWifi-1.6, verified against Kismet's `kis_wiglecsvlogfile.cc` source, not guessed) and `storage::kml*` (verified against Google's KML reference — coordinate order, `aabbggrr` color order). 35 host tests, including a real XML-well-formedness cross-check (`tools/check_wardrive_kml.py`, same role `check_ocp_text.py` plays for the C field encoder). **Not done:** no SD file I/O (no `wardrive_logger.cpp` — the lora_logger.cpp equivalent doesn't exist yet), nothing feeding these from `ScanModel`+`GnssModel`, no **Drive** view. `GnssModel` now parses RMC's date field (closed same day, see below) so a real calendar timestamp exists to hand `wardriveCsvRow()`'s FirstSeen, but nothing wires that connection yet — the row writer still takes it as an explicit parameter. Not hardware-confirmed at all.
