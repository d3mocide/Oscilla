...
| `cc1101_radio.c` | CC1101 driver layer (RX path only): manual reset, PARTNUM/VERSION probe, OOK modem config, RXBYTES/FIFO poll task (no GDO0 wiring in this harness — polling, not IRQ-driven) |
| `legacy_recon.c` | CC1101 OCP verb glue: raw byte/RSSI capture, no vendor protocol decode yet. **No TX path** (§8) |
| `subghz_arbiter.c` | **Single-owner sub-GHz bus arbitration** between the Wio and CC1101 (§6.2) — structural mirror of `radio_arbiter.c` |
...
- One CC1101 poll task; no ISR (GDO0 unconnected in this harness), reads RXBYTES on a timer instead.
...
| `src/ui/sweep_view` · `src/ui/trace_view` · `src/ui/dwell_animation` | view | AP list; one AP in depth; shared bounded-listen progress |
| `src/ui/chrome` | view | Shared deck header, transport strip, expanded body budget, status/battery indicators, and transient notices |
| `src/ui/theme` | view | Brand semantic palette and fixed 240×135 layout budget |
| `src/ui/boot_screen` | view | Short branded startup identity using the Lens Core mark and deck wordmark |
| `src/ui/help_view` | view | Shared `h`-key keyboard reference card |
...
| View | Verb(s) | Shows |
|---|---|---|
| **Wi-Fi Scan** | `scan_networks` / `start_wifi_scan` | AP list: SSID · ch · band · sec · RSSI bar. Bounded scan (`r`) or continuous first-sighting discovery (`c`). Select → AP Detail after stopping live mode. |
