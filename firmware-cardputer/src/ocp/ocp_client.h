...
public:
     ...
     * they're meaningful running totals, not restart artifacts. Called by
     * handleHello() itself on every successful handshake — public so a
     * caller can also invoke it explicitly if needed, but nothing needs to:
     * it isn't conditioned on the onReset() callback firing, which only
     * covers the narrower unsolicited-while-Ready shape (see WORKLOG). */
