...
def main(test_bin: str) -> int:
    sample = subprocess.run([test_bin, "--sample"], capture_output=True, text=True, check=True).stdout

    if re.search(r"<href>\s*https?://", sample):
        print("FAIL: KML contains a remote icon/resource URL", file=sys.stderr)
        return 1
    ...
    if len(placemarks) != 4:
        print(f"FAIL: expected 4 Placemarks (1 track + 3 APs), found {len(placemarks)}", file=sys.stderr)
        return 1
    ...
    recovered = subprocess.run([test_bin, "--recovery"], capture_output=True, check=True).stdout.split(b"\x1e")
    if recovered and recovered[-1] == b"": recovered.pop()
    for i, raw in enumerate(recovered):
        try:
            ET.fromstring(raw)
        except ET.ParseError as e:
            print(f"FAIL: recovery prefix {i} is not well-formed XML: {e}", file=sys.stderr)
            return 1

    print(f"  wardrive KML: well-formed, {len(placemarks)} placemarks, {len(styles)} styles, {len(recovered)} recovery prefixes")
    return 0
